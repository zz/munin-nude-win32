...
#endif
			_Module.LogEvent("ConsolePipe:Execute -> Unable to create console window");
			return CPEXEC_MISC_ERROR;
		}
	}
                        ...
                        DUPLICATE_SAME_ACCESS))
	{
		...
		if(LaunchRedirected(pszCommand, loadzaHandles[1], loadzaHandles[3], loadzaHandles[4]))
		{
			...
			if(m_hListenerThread)
				...
				_Module.LogEvent("ConsolePipe:Execute -> unable to CreateThread (error code %x)", errorCode);
				status = CPEXEC_MISC_ERROR; // rare
			}
		}
		else {
			_Module.LogEvent("ConsolePipe:Execute -> LaunchRedirected failed");
			status = CPEXEC_COMMAND_ERROR;
		}
	}
	else
	{
		_Module.LogEvent("ConsolePipe:Execute -> cannot create pipe or duplicate handle");
...
void CConsolePipe::Cleanup() {
	if(m_hListenerThread) {
...
#ifdef TRY2
			m_bKillSwitch = true;
			if (INVALID_HANDLE_VALUE != m_hOutputRead)
		...
		//else //yudhi: always close handle whatever happened
